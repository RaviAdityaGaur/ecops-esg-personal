...
const UserAssignmentDialog: React.FC<UserAssignmentDialogProps> = ({ open, onClose, onAssign }) => {
  ...
  const filteredEmployees = employees.filter(
    (employee) =>
      ...
      employee.empId.includes(searchQuery) ||
...
const EditTaskDialog: React.FC<EditTaskDialogProps> = ({ open, onClose, task, onSave }) => {
  ...
  React.useEffect(() => {
    if (task) {
      ...
      if (task.description) {
        ...
        const defaultDescription = task.id === 1 ? 
          "Qualitative/Quantitative\nExample - Please prove data for carbon Emission." : 
  ...
  return (
    ...
    >
      <DialogTitle sx={{ fontWeight: 600, fontSize: "20px", py: 2 }}>
        Edit Task
      </DialogTitle>
      
      <DialogContent sx={{ pt: 0 }}>
        <Typography variant="subtitle2" sx={{ mb: 1, color: "text.secondary" }}>Task Title</Typography>
        <TextField
          fullWidth
          value={title}
...
interface Task {
  ...
  status: string
  priority: string
  column: "notComplete" | "completed" | "approved"
                        ...
                        <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
                          <Chip
                            ...
                            sx={{
                              ...
                              cursor: "pointer",
                              bgcolor:
                                task.priority === "high" ? "#fce8e8" : task.priority === "medium" ? "#fff8e1" : "#e6f4ea",
                              color:
                                task.priority === "high" ? "#d93025" : task.priority === "medium" ? "#b06000" : "#137333",
                            }}
                          />
                          <Box sx={{ display: "flex", alignItems: "center" }}>
                            <Chip
                              label={task.assignedTo ? task.dueDate : ""}
                              size="small"
                              sx={{
                                height: "24px",
...
type ColumnId = "notComplete" | "completed" | "approved"
      ...
      prevTasks.map((task) =>
        task.id === taskId
          ? {
              ...task,
              column: targetColumn,
              status: targetColumn === "completed" || targetColumn === "approved" ? "completed" : task.status,
            }
          : task,
      ),
    )
    ...
    setTasks(prevTasks => prevTasks.map(task => {
      ...
      return {
        ...
        description: task.id === 1 ? 
          "Qualitative/Quantitative\nExample - Please prove data for carbon Emission." : 
          "Specify which information is unavailable or incomplete. When the information is incomplete, specify which part is missing..."
      };
    }));
  }, []);
          ...
          <Box sx={{ width: "100%", bgcolor: "background.paper", borderRadius: 2, boxShadow: "none", p: 1.5, mb: 2 }}>
            ...
            <Collapse in={expanded}>
              <Typography variant="body2" color="text.secondary">
                Efficient task management helps teams stay organized, prioritize work, and meet deadlines effectively. By
                structuring tasks and workflows, organizations can improve productivity and ensure smoother collaboration.
                Please select the task management tools you want to use for better efficiency.
              </Typography>
            </Collapse>
          </Box>
                ...
                >
                  ...
                  >
                    <Typography variant="h6" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1 }}>
                      Not Started
                      <Chip 
                        label={getColumnTasks("notComplete").length} 
                        size="small" 
                        sx={{ 
                          height: "20px", 
                          bgcolor: "#76B8F6",
                  ...
                  >
                    <StrictModeDroppable droppableId="notComplete">
                      {(provided: DroppableProvided) => (
                        ...
                        >
                          <TaskList tasks={getColumnTasks("notComplete")} onOpenAssignDialog={handleOpenDialog} />
                ...
                >
                  ...
                  >
                    <Typography variant="h6" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1 }}>
                      Completion approved
                      <Chip 
                        label={getColumnTasks("approved").length} 
                        size="small" 
                        sx={{ 
                          height: "20px", 
                          bgcolor: "#40C79A",
                  ...
                  >
                    <StrictModeDroppable droppableId="approved">
                      {(provided: DroppableProvided) => (
                        ...
                        >
                          <TaskList tasks={getColumnTasks("approved")} onOpenAssignDialog={handleOpenDialog} />
                          {provided.placeholder}
                        </div>
                      )}
                    </StrictModeDroppable>
