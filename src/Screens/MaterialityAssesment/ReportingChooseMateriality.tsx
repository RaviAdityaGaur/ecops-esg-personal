...
export default function ReportingChooseMateriality() {
  const { reportId } = useParams<{ reportId: string }>();
  const navigate = useNavigate();
  const [selectedMateriality, setSelectedMateriality] =
    useState<MaterialityType>(null);
  ...
  const handleMaterialitySelect = async (materialityType: MaterialityType) => {
    ...
    console.log("materialityType", materialityType);
    /* try {
      await api.post(`esg/api/surveys/${reportId}/set_materiality_type/`, {
        json: {
      ...
      navigate(`/choose-standard/${reportId}`);
    } catch (error) {
