...
interface ReportingRequirement {
    ...
    apiRowId?: number | null; // Original row ID from API for updates (integer)
}
...
const DataEntryForm: React.FC = () => {
    ...
    const [reportingRequirements, setReportingRequirements] = useState<ReportingRequirement[]>([]);
    ...
    // Function to refresh template data after updates
    const refreshTemplateData = async () => {
        try {
            const templateInstanceId = currentTask?.template_instance?.id;
            if (templateInstanceId) {
                const templateResponse: any = await api.get(`esgtemplate/template-instance/${templateInstanceId}/data`).json();
                setTemplateInstanceData(templateResponse);
    ...
    const convertTemplateDataToRequirements = (rows: any) => {
        ...
        // Extract all unique periods from the data to create dynamic columns
        ...
        Object.keys(rows).forEach((rowKey) => {
            ...
            if (row.data && Array.isArray(row.data)) {
                row.data.forEach((dataItem: any) => {
                    if (dataItem.period) {
                        allPeriods.add(dataItem.period);
                    }
                });
            }
        });
        
        // Convert periods to sorted array for consistent column order
        const columns = Array.from(allPeriods).sort();
        setDynamicColumns(columns);
        
        // Create empty months object based on dynamic columns
        const emptyMonths = columns.reduce((acc, period) => ({ ...acc, [period]: '' }), {});
        
        // Helper function to populate months data from API
        const populateMonthsFromData = (data: any[]) => {
            const months = { ...emptyMonths };
            data.forEach((dataItem: any) => {
                if (dataItem.period && dataItem.value !== undefined) {
                    months[dataItem.period] = dataItem.value;
                }
            });
            return months;
        };
        ...
        // Convert grouped rows to requirements with proper numbering and API data
        Object.keys(groupedRows).forEach((composition) => {
            const rowGroup = groupedRows[composition];
            
            // Find if there's a main row for this composition
            const mainRow = rowGroup.find(row => row.composition === composition && (!row.sub_composition || row.sub_composition === composition));
            
            // Add main composition as a numbered requirement (1, 2, 3...)
            const mainRowKey = Object.keys(rows).find(key => rows[key] === mainRow);
            const apiRowId = mainRow ? parseInt(mainRowKey || '0') : null;
            
            console.log('Converting row for composition:', composition, {
                mainRowKey,
                apiRowId,
                mainRowData: mainRow?.data
            });
            ...
            requirements.push({
                ...
                unit: 'UNIT',
                months: mainRow && mainRow.data ? populateMonthsFromData(mainRow.data) : { ...emptyMonths },
                rowType: 'main',
                mainIndex: mainIndexCounter,
                apiRowId: apiRowId // Store original API row ID as integer
            });
            
            // Note: Sub-compositions are excluded - only showing main headings
            
            mainIndexCounter++;
        });
        
        // If no rows found, return default structure with dynamic columns
    ...
    const handleCellValueChange = (requirementId: number, month: string, value: string) => {
        console.log('Cell value changed:', { requirementId, month, value });
        ...
        // Call API immediately for testing (no debouncing for now)
        console.log('Calling updateCellValueInAPI...');
        updateCellValueInAPI(requirementId, month, value);
    };

    // Separate function to handle API updates
    const updateCellValueInAPI = async (requirementId: number, month: string, value: string) => {
        try {
            // Find the requirement to get the API row ID
            const requirement = reportingRequirements.find(req => req.id === requirementId);
            if (!requirement || !requirement.apiRowId) {
                console.error('Could not find API row ID for requirement:', requirementId);
                console.log('Available requirements:', reportingRequirements.map(r => ({ id: r.id, apiRowId: r.apiRowId, description: r.description })));
                return;
            }

            // Get the template instance ID
            const templateInstanceId = currentTask?.template_instance?.id;
            if (!templateInstanceId) {
                console.error('Template instance ID not found');
                return;
            }
            ...
            console.log('Updating single cell - API call data:', JSON.stringify(updateData, null, 2));
            console.log('Full API URL:', `${api.prefixUrl}esgtemplate/template/${templateInstanceId}/update-data/`);
            console.log('Template Instance ID:', templateInstanceId);

            // Call the update API
            const response = await api.post(`esgtemplate/template/${templateInstanceId}/update-data/`, {
                json: updateData
            }).json();

            console.log('Successfully updated data for row:', requirement.apiRowId, 'period:', month, 'value:', value);
            ...
            console.error('Error details:', {
                ...
                apiRowId: reportingRequirements.find(req => req.id === requirementId)?.apiRowId
            });
    ...
    // Function to submit all table data to the update API
    const submitTableData = async (templateInstanceId: number) => {
        try {
            // Prepare all rows data for submission
            const rowsData = reportingRequirements
                .filter(req => req.apiRowId) // Only include rows that have API row IDs
                .map(req => {
                    // Convert months object to data array
                    const data = Object.entries(req.months)
                        .filter(([period, value]) => value && value.trim() !== '') // Only include non-empty values
                        .map(([period, value]) => ({
                            period,
                            value
                        }));
                ...
                .filter(row => row.data.length > 0); // Only include rows that have data
            ...
            if (rowsData.length > 0) {
                ...
                console.log('Submitting table data to API:', JSON.stringify(updateData, null, 2));

                await api.post(`esgtemplate/template/${templateInstanceId}/update-data/`, {
                    json: updateData
                }).json();

                console.log('Successfully submitted all table data:', rowsData);
            } else {
                console.log('No table data to submit (all fields are empty)');
            }
        } catch (error) {
            console.error('Error submitting table data:', error);
            throw error; // Re-throw to be caught by the main submit function
        }
    };
    ...
    const submitFormData = async (status: 'draft' | 'submitted') => {
        try {
            ...
            if (!templateInstanceId) {
                throw new Error('Template instance ID not found');
            }

            // Prepare the disclosure data according to the API specification
            const disclosureData = {
                template_instance: templateInstanceId,
                summary: summary, // HTML content from summary editor
                disclosure_3_3_summary: disclosureContent, // HTML content from disclosure editor
            };

            // POST to the template disclosure API endpoint
            const disclosureResponse = await api.post(`esgtemplate/template-instance/${templateInstanceId}/disclosure/`, {
                json: disclosureData
            }).json();

            // Also submit all table data to the update API
            await submitTableData(templateInstanceId);
    ...
    // Use only dynamic columns from API data - no fallback to mock data
    const monthColumns = dynamicColumns.length > 0 ? dynamicColumns : [];
                        ...
                        <Paper sx={{ p: 3, borderRadius: 2 }}>
                            ...
                            {activeTab === 'requirements' && (
                                <Box>
                                    ...
                                    {/* COMMENTED DISPLAY-ONLY VERSION:
                                    ...
                                    */}
                                    ...
                                    <Typography variant="caption" sx={{ display: 'block', mb: 1, color: 'text.secondary' }}>
                                        Debug: monthColumns: {monthColumns.length}, requirements: {reportingRequirements.length}
                                    </Typography>
                                    
                                    {monthColumns.length > 0 && reportingRequirements.length > 0 ? (
                                        <TableContainer component={Paper} variant="outlined" sx={{ mb: 3 }}>
                                            <Table size="small">
                                                <TableHead>
                                                    <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                                                        ...
                                                        {monthColumns.map((month) => (
                                                            <TableCell key={month} sx={{ textAlign: 'center' }}>
                                                                <TextField
                                                                    ...
                                                                    onChange={(e) => {
                                                                        console.log('Input changed:', req.id, month, e.target.value);
                                                                        handleCellValueChange(req.id, month, e.target.value);
                                                                    }}
                                                                    variant="outlined"
                                                                    sx={{
                                                                        width: '60px',
                                        ...
                                        <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center', py: 3 }}>
                                            Loading reporting requirements data...
                                        </Typography>
                                    )}
