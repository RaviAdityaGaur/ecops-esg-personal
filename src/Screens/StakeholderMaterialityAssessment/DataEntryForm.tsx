...
import { Box, Typography, Paper, Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Grid, Chip, IconButton, Select, MenuItem, FormControl, Tooltip } from '@mui/material';
...
const fontWhitelist = ['arial', 'comic-sans-ms', 'courier-new', 'georgia', 'helvetica', 'lucida-sans-unicode', 'tahoma', 'times-new-roman', 'trebuchet-ms', 'verdana'];

// --- Custom Toolbar Configuration for React-Quill ---
const quillModules = {
  toolbar: [[{ font: fontWhitelist }], ['bold', 'italic', 'underline'], [{ header: [1, 2, 3, false] }], [{ list: 'ordered' }, { list: 'bullet' }], [{ align: [] }], ['link', 'image'], [{ color: [] }, { background: [] }], ['clean']]
};
...
const DataEntryForm: React.FC = () => {
  ...
  const handleCellValueChange = (requirementId: number, month: string, value: string) => {
    ...
    // setReportingRequirements(prev => prev.map(req => (req.id === requirementId ? { ...req, months: { ...req.months, [month]: value } } : req)));
  };
      ...
      <Box sx={{ width: '100%', p: 3, bgcolor: '#f7f8fc' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, px: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            <Button variant="text" onClick={handleCustomiseTemplate} sx={{ textTransform: 'none', color: 'text.primary', fontWeight: 600 }}>
              Customise Template
            </Button>
            <Button variant="text" sx={{ textTransform: 'none', color: 'text.primary', fontWeight: 600 }}>
              Create Custom Template
            </Button>
            <Button variant="outlined" onClick={handleRequestDataFromOthers} sx={{ textTransform: 'none', borderColor: '#147C65', color: '#147C65', '&:hover': { borderColor: '#0f5a4a' } }}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 3, borderRadius: 2 }}>
              ...
              <Box sx={{ mb: 3, '.ql-editor': { minHeight: '100px' } }}>
                <ReactQuill theme="snow" value={disclosureContent} onChange={setDisclosureContent} placeholder="Enter disclosure content here..." modules={quillModules} />
              </Box>

              <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
                <Box sx={{ display: 'flex', gap: 3 }}>
                  {['Requirements', 'Recommendations', 'Guidance'].map(tab => (
                    <Button key={tab} onClick={() => setActiveTab(tab.toLowerCase() as any)} sx={{ textTransform: 'capitalize', color: activeTab === tab.toLowerCase() ? '#147C65' : 'text.secondary', borderBottom: activeTab === tab.toLowerCase() ? 2 : 0, borderColor: '#147C65', borderRadius: 0, px: 0, py: 1 }}>
                      {tab}
                    </Button>
                  ))}
                </Box>
              ...
              {activeTab === 'requirements' && (
                <Box>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                    This section will display the GRI requirements in a read only format...
                  </Typography>
                      ...
                      <TableHead>
                        <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                          ...
                          {monthColumns.map(month => (
                            <TableCell key={month} sx={{ fontWeight: 600, width: 80 }}>
                              {month}
                            </TableCell>
                          ))}
                        </TableRow>
                      ...
                      <TableBody>
                        {reportingRequirements.map(req => (
                          <TableRow key={req.id}>
                            ...
                            <TableCell sx={{ fontSize: '14px', fontWeight: 500, color: 'text.secondary' }}>{req.unit}</TableCell>
                            {monthColumns.map(month => (
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 3, borderRadius: 2 }}>
              ...
              <Box sx={{ '.ql-editor': { minHeight: '150px' } }}>
                <ReactQuill theme="snow" value={summary} onChange={setSummary} placeholder="Enter summary here..." modules={quillModules} />
              </Box>
            </Paper>
          </Grid>
          ...
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', mt: 2 }}>
              ...
              <Button variant="outlined" onClick={handleSaveAsDraft} sx={{ textTransform: 'none', borderColor: '#147C65', color: '#147C65', '&:hover': { borderColor: '#147C65', bgcolor: 'rgba(20, 124, 101, 0.05)' } }}>
                Save as draft
              </Button>
              <Button variant="contained" onClick={handleSubmit} sx={{ textTransform: 'none', bgcolor: '#147C65', '&:hover': { bgcolor: '#0f5a4a' } }}>
                Submit
              </Button>
            </Box>
          </Grid>
