    ...
    const fetchData = async () => {
      try {
        ...
        const currentSurvey = surveyMaterialDataList.find(
          (item) => item.survey === parseInt(reportId)
        );
        ...
        const data = await api
          .get(`esg/api/disclosures/list-for-mapping/?survey_id=${reportId}`)
          .json();
        ...
        console.log("processedDisclosures", processedDisclosures)
        setAllDisclosures(processedDisclosures)
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      for (const row of selectedDisclosures) {
        ...
        await api.post("esg/api/map-other-disclosures-to-report/", {
          json: {
            report_id: reportId,
            disclosure_ids: Object.keys(responseMap)
          },
        });
