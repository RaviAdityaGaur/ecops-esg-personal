...
import { CircularProgress, Box, Typography, IconButton, Tooltip, Button } from '@mui/material';
...
const ViewReports = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const periodsResponse = await api.get('esg/api/esg-reporting-period/').json();
        setReportingPeriods(periodsResponse);

        // Then fetch and process reports
        const reportsResponse = await api.get('esg/api/reporting/').json();
        const enhancedData = reportsResponse.map(report => {
          ...
          const matchingPeriod = periodsResponse.find(period => period.id === report.reporting_period);
          const formattedReportingPeriod = matchingPeriod ? `${matchingPeriod.name} (${matchingPeriod.type_of_year} ${matchingPeriod.year})` : '-';
  ...
  // funtion edit button
  // reporting-choose-standard/59

  const handleEdit = (reportId) => {
    navigate(`/reporting-choose-standard/${reportId}`);
  };

  const handleConfigure = reportId => {
    navigate(`/report-configure/${reportId}`);
  };

  const handleAdd = reportId => {
    navigate(`/report-add/${reportId}`);
  };
  ...
  const columns = [
    {
      ...
      renderCell: params => (
        <Box sx={{ display: 'flex', alignItems: 'center', height: '100%' }}>
          <Typography sx={{ color: '#147C65', fontWeight: 500 }}>{params.value}</Typography>
        </Box>
      )
    },
    // Reporting period
      ...
      renderCell: params => (
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <Tooltip title="Configure" arrow>
            <IconButton
              onClick={() => handleConfigure(params.row.id)}
              sx={{
                color: '#147C65',
                '&:hover': {
                  backgroundColor: 'rgba(20, 124, 101, 0.04)'
  ...
  return (
    <SidebarHeader title="All Reports">
      <Box p={3} sx={{ backgroundColor: '#fff', borderRadius: '10px' }} height="70vh">
        {/* Buttons above the table */}
        <Box mb={2} display="flex" justifyContent="flex-end" gap={2}>
          {/* <Typography variant="contained" color="primary" onClick={() => navigate('/reports')}>
            Add Report
          </Typography> */}
          <Button
            variant="contained"
