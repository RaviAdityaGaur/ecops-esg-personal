import { Box, Typography, Button, Tabs, Tab, Switch, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
const SelectedDisclosuresSummary = () => {
  ...
  //   );
  // };
  const handleResponseChange = async (rowId: number, value: 'yes' | 'no' | string) => {
  ...
  const columns = [
    {
      ...
      renderCell: params => (
        <Box
          sx={{
            pl: 2,
            color: missingDataDisclosures.responses.includes(params.row.id) || missingDataDisclosures.riskCategories.includes(params.row.id) ? '#DF0404' : 'inherit',
            fontWeight: missingDataDisclosures.responses.includes(params.row.id) || missingDataDisclosures.riskCategories.includes(params.row.id) ? 'bold' : 'normal'
          }}
        >
          {params.value}
        </Box>
    ...
    },
    {
      ...
      renderCell: params => {
        ...
        return (
          ...
          >
            <Switch checked={isAdded} onChange={handleToggle} color="success" size="medium" />
          </Box>
        );
      }
    }
